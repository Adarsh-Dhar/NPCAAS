      ...
      <NexusLayout>
        <div className="flex flex-col h-full overflow-auto">
          <div className="border-b border-border/20 px-6 py-5 flex-shrink-0 bg-gradient-to-r from-primary/5 to-secondary/5 backdrop-blur-sm rounded-lg mb-6">
            ...
            <p className="text-xs text-muted-foreground mt-2 ml-7">Trade not found in the live server snapshot</p>
          </div>
      ...
      <div className="flex flex-col h-full overflow-auto">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto pr-4">
          <div className="border border-border/40 rounded-lg p-5 bg-card/50 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-5">
              <div className="bg-muted/30 rounded p-3">
                ...
                <div className="text-primary font-bold text-lg">{trade.amount || trade.price || '0'}</div>
              </div>
            ...
            <div className="pt-5 border-t border-border/20">
              ...
              <div className="space-y-2">
                {(trade.agents.length > 0 ? trade.agents : trade.brokerName ? [trade.brokerName] : []).map((agent) => (
          ...
          <div className="border border-border/40 rounded-lg p-5 bg-card/50 backdrop-blur-sm">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">{trade.details || trade.asset || 'Live settlement record'}</p>
          </div>
            ...
            <div className="space-y-2 text-xs text-muted-foreground">
              {trade.status === 'SETTLED' ? (
                <>
                  ...
                  <div className="flex items-center gap-2 p-2 bg-accent/10 rounded hover:bg-accent/20 transition-colors">
                    <div className="w-1.5 h-1.5 bg-accent rounded-full flex-shrink-0" />
                    <p className="font-mono text-xs">Hash: {trade.txHash ?? 'pending'}</p>
                  </div>
                </>
              ) : trade.status === 'ACTIVE' ? (
                <>
